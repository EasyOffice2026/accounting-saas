...
export default function VendorsPage() {
  ...
  useEffect(() => { if (user?.org_id) loadVendors(); }, [user?.org_id]);

  async function loadVendors() {
    const { data } = await supabase.from('acct_vendors').select('*').eq('org_id', user!.org_id).order('name');
    setVendors(data || []);
  }
  ...
  function openEdit(v: Vendor) {
    setEditing(v);
    setForm({ name: v.name, email: v.email || '', phone: v.phone || '', address: v.address || '', tax_id: v.tax_id || '', contact_name: v.contact_name || '' });
    setShowDialog(true);
  }
  ...
  async function handleSave() {
    ...
    if (editing) {
      await supabase.from('acct_vendors').update(data).eq('id', editing.id);
    } else {
      await supabase.from('acct_vendors').insert(data);
    }
    setShowDialog(false);
    loadVendors();
  }
  ...
  async function handleDelete(id: string) {
    ...
    await supabase.from('acct_vendors').delete().eq('id', id);
    loadVendors();
  }

  const filtered = vendors.filter(v => !search || v.name.toLowerCase().includes(search.toLowerCase()) || v.email?.includes(search));
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-8 text-gray-400">No vendors yet</td></tr>
            ) : filtered.map(v => (
              <tr key={v.id} className="border-b border-gray-50 hover:bg-gray-50">
                <td className="py-3 px-4 font-medium text-gray-900">{v.name}</td>
                <td className="py-3 px-4 text-gray-600">{v.contact_name || '-'}</td>
                <td className="py-3 px-4 text-gray-600">{v.email || '-'}</td>
                <td className="py-3 px-4 text-gray-600">{v.phone || '-'}</td>
                <td className="py-3 px-4 text-right font-medium">{formatCurrency(Number(v.outstanding_balance) || 0)}</td>
                <td className="py-3 px-4 text-center">
                  <div className="flex items-center justify-center gap-1">
                    <button onClick={() => openEdit(v)} className="p-1.5 rounded-lg hover:bg-blue-50 text-blue-600"><Pencil size={15} /></button>
                    <button onClick={() => handleDelete(v.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-red-600"><Trash2 size={15} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {showDialog && (
        <div className="dialog-overlay" onClick={() => setShowDialog(false)}>
          <div className="dialog-content max-w-lg animate-fadeIn" onClick={e => e.stopPropagation()}>
            <h2 className="text-xl font-semibold text-gray-900 mb-5">{editing ? t('edit') : t('addVendor')}</h2>
